  ...
  return (
    <div className="h-[calc(100vh-160px)] flex flex-col gap-10 animate-in fade-in duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          <h1 className="text-3xl font-extrabold text-white uppercase italic tracking-tight">Retail Hub (POS)</h1>
          <p className="text-text-dim">Operational node for direct supply-to-athlete distribution.</p>
        </div>
      ...
      <div className="flex-1 flex gap-10 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col gap-8">
           <div className="relative">
              <Search className="absolute right-6 top-1/2 -translate-y-1/2 h-6 w-6 text-text-dim" />
              <input 
                placeholder="Scan Asset ID or Name..." 
                className="w-full bg-white/2 border border-white/5 rounded-[2rem] py-6 pr-16 pl-8 text-lg font-bold text-white focus:outline-none focus:border-accent/40 shadow-2xl"
              />
           </div>
           ...
           <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto custom-scrollbar pr-2">
              {inventory.map((product) => (
                <button 
                  ...
                  className="glass p-8 rounded-[2.5rem] border border-white/5 text-right transition-all hover:border-accent/40 hover:bg-white/[0.04] group relative overflow-hidden"
                >
                   ...
                   <p className="text-xs font-bold text-text-dim mb-6 uppercase tracking-tighter">Stock: {product.stock} Units</p>
        ...
        <div className="w-[450px] glass rounded-[3rem] border border-white/5 flex flex-col shadow-2xl overflow-hidden relative">
           ...
           <div className="p-10 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
              <div className="flex items-center gap-4">
                 ...
                 <h2 className="text-xl font-black text-white uppercase italic tracking-tight">Active Loadout</h2>
              </div>
              <span className="text-xs font-bold text-text-dim bg-white/5 px-3 py-1 rounded-full">{cart.length} Items</span>
           </div>
           ...
           <div className="flex-1 overflow-y-auto custom-scrollbar p-10 space-y-8">
              {cart.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center gap-6 opacity-20">
                   ...
                   <p className="text-xs font-black uppercase tracking-[0.3em]">Manifest Empty</p>
                </div>
              ) : (
                cart.map((item) => (
                  <div key={item.id} className="flex gap-6 group">
                     ...
                     <div className="flex flex-col items-end gap-3">
                        ...
                        <button onClick={() => removeFromCart(item.id)} className="text-[10px] font-black text-red-500/40 uppercase tracking-widest hover:text-red-500 transition-all">Flush Item</button>
                     </div>
                  </div>
                ))
              )}
           ...
           <div className="p-10 bg-white/[0.02] border-t border-white/5 space-y-8">
              <div className="space-y-4">
                 ...
                 <div className="flex justify-between items-center pt-4 border-t border-white/5">
                    <span className="text-sm font-black text-white/40 uppercase italic tracking-[0.2em]">Total Payload</span>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <button className="flex flex-col items-center justify-center gap-3 p-6 rounded-[2rem] bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 transition-all group">
                    ...
                    <span className="text-[10px] font-black text-text-dim uppercase tracking-widest">Liquid Cash</span>
                 </button>
                 <button className="flex flex-col items-center justify-center gap-3 p-6 rounded-[2rem] bg-accent/10 border border-accent/20 hover:bg-accent/20 transition-all group">
                    ...
                    <span className="text-[10px] font-black text-accent uppercase tracking-widest">Digital Token</span>
                 </button>
              </div>
